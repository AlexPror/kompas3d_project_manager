...
## [2.3.0] - 03.11.2025 - Обновление штампа "Разработчик" + Покупные компоненты

### ✨ Новые возможности

- 📋 **Автоматическое обновление штампа "Разработчик"**
  - Добавлено поле ввода "Разработчик (опционально)" в секцию обновления переменных
  - Новая кнопка "📋 Обновить штампы чертежей (Разработчик)" для быстрого обновления
  - Автоматическое обновление ячейки 110 штампа во всех чертежах проекта
  - Интеграция с функцией "ПОЛНЫЙ ЦИКЛ" - штампы обновляются автоматически

### 🔧 Исправления

- ⚙️ **Корректная обработка покупных компонентов**
  - **Теплообменник**: обозначение в сборке не меняется, в детали обновляется длина (L1-300), переменная L1 обновляется
  - **Вентилятор**: обозначение не меняется ни в сборке, ни в детали
  - **Блок электропитания**: обозначение не меняется ни в сборке, ни в детали
  - Обычные детали: обозначение обновляется по стандартной схеме (ZVD.LITE.H.B1.XXX)

### 🔧 Технические детали

- Обновлен `DrawingAutoUpdater.update_all_drawings()` - добавлен параметр `developer`
- Добавлен метод `KompasManagerGUI.update_drawing_stamps()` для отдельного обновления штампов
- Использование KOMPAS API: `GetStamp()`, `ksOpenStamp()`, `ksColumnNumber(110)`, `ksSetStampColumnText()`
- Поддержка всех типов чертежей (сборочные и деталировочные)

### 📖 Документация

- Создан файл `FEATURE_DEVELOPER_STAMP.md` с подробным описанием функциональности
- Примеры использования и технические детали API

### 🎯 Использование

1. **Полный цикл**: Заполните поле "Разработчик" перед запуском полного цикла
2. **Только штампы**: Используйте кнопку "📋 Обновить штампы чертежей" для обновления существующих чертежей

---

## [2.2.1] - 28.10.2025 - Hotfix: Стабильность COM-соединения

### 🔧 Исправления

- **Исправлена потеря COM-соединения с КОМПАС-3D**
  - Ошибка: "Объект не подключен к серверу" при выполнении полного цикла
  - Добавлено автоматическое переподключение с проверкой живости соединения
  - Добавлен параметр `force_reconnect` в метод `connect_to_kompas()`
  - Теперь можно создавать несколько проектов подряд без перезапуска

- **Улучшена стабильность работы**
  - Все компоненты закрывают открытые документы перед началом работы
  - Добавлены паузы для стабилизации между операциями
  - Попытка использовать существующий экземпляр КОМПАС или создать новый

### 📦 Обновленные компоненты

- `BaseKompasComponent` - улучшенный метод подключения
- `CascadingVariablesUpdater` - принудительное переподключение
- `DesignationUpdaterFixed` - принудительное переподключение
- `DrawingAutoUpdater` - принудительное переподключение
- `UnfoldingDxfExporter` - принудительное переподключение
- `DrawingExporter` - принудительное переподключение

---

## [2.2.0] - 28.10.2025 - Template System Edition
...
- 📚 **Система управления шаблонами**
  ...
  - Метаданные: название, параметры, теги, статистика
  - Автоматическое создание README для каждого шаблона
  - Интеллектуальная фильтрация при копировании (временные файлы, архивы)

- 📁 **Организация BMP файлов**
  - Автоматическое создание папки BMP/
  - Перемещение всех BMP файлов в отдельную папку
  - Интеграция в полный цикл работы

- 🔍 **Анализатор проектов**
  - Детальное изучение структуры проекта
...
- Полный цикл расширен до **9 шагов** (добавлена организация BMP)
- Кнопка "Сохранить как шаблон" в интерфейсе
- Улучшенная документация системы шаблонов

### 📦 Новые компоненты

- `BmpOrganizer` - организация BMP файлов
- `TemplateManager` - управление библиотекой шаблонов
- `ProjectTemplateAnalyzer` - анализ структуры проектов

### 📚 Документация

- Добавлен `TEMPLATE_SYSTEM.md` - полное руководство по работе с шаблонами
- Добавлен `project_template_analyzer.py` - инструмент анализа
- Обновлен `README.md` с описанием новых возможностей

### 🎯 Стратегия

- Итеративный подход: создание шаблонов из реальных проектов
- Накопление библиотеки проверенных конфигураций
- Постепенная оптимизация и отсев избыточных шаблонов
